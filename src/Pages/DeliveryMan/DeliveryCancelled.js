...
import DeliveryCancelledTable from "../../Components/Tables/DeliveryMan/DeliveryCancelledTable";
import { DeliveryContext } from "../../Contexts/DeliveryContext/DeliveryProvider";

const DeliveryCancelled = () => {
  const [selectedRiders, setSelectedRiders] = useState([]);
  const [cancelledRiders, setCancelledRiders] = useState([]);
  const {
    isLoading,
    fetchRiders,
    searchBarValue,
  ...
  const handleSelectCheckbox = (riderId, e) => {
    const selectedRidersList = [...selectedRiders];
    if (e.target.checked) {
      selectedRidersList.push(riderId);
    } else {
      const index = selectedRidersList.indexOf(e.target.value);
      if (index > -1) {
  ...
  const handleUserTypeToggle = (userType) => {
    filterRidersByUserType(userType);
  };
  const handleLocationTypeToggle = (locationType) => {
    filterRidersByLocationType(locationType);
  };

  useEffect(() => {
    const filteredRidersByStatus = filteredRidersBySearch?.filter(
      (rider) => rider?.rider_status?.toLowerCase() === "cancelled"
    );
    setCancelledRiders(filteredRidersByStatus);
  }, [filteredRidersBySearch]);
